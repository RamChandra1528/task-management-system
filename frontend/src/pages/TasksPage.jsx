...
import { Filter, LayoutGrid, List, MoreVertical } from "lucide-react";
...
export default function TasksPage() {
  ...
  const selectedTask =
    visibleTasks.find((task) => task._id === selectedId) || visibleTasks[0] || null;
  ...
  return (
    <div className="space-y-6">
      <SectionTitle title="Tasks" subtitle="Manage and track all your tasks in one place" />
            ...
            <div className="divide-y divide-brand-100">
              {visibleTasks.slice(0, 10).map((task) => (
                <button
                  ...
                  className={`grid w-full gap-4 px-5 py-4 text-left transition lg:grid-cols-[36px_1.6fr_1.2fr_1.1fr_0.9fr_0.9fr_0.9fr_40px] ${
                    selectedTask?._id === task._id ? "bg-brand-50/60" : "bg-white hover:bg-brand-50/30"
                  }`}
                >
